"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
const supplierModel_1 = __importDefault(require("../models/supplierModel"));
const getSuppliers = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield supplierModel_1.default.getSuppliers();
    return res.json(result);
});
const getSupplierById = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield supplierModel_1.default.getSupplierById(req.params.supplier_id);
    return res.json(result);
});
const postSupplier = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield supplierModel_1.default.postSupplier(req.body);
    return res.json(result);
});
const putSupplier = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield supplierModel_1.default.putSupplier(req.body);
    return res.json(result);
});
const deleteSupplier = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield supplierModel_1.default.deleteSupplier(req.body);
    return res.json(result);
});
module.exports = {
    getSuppliers: getSuppliers,
    getSupplierById: getSupplierById,
    postSupplier: postSupplier,
    putSupplier: putSupplier,
    deleteSupplier: deleteSupplier,
};
